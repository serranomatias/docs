...
<Note>
  To export your whole contact base, iterate with `limit=1000` and increase `offset` until you have fetched `pagination.total_rows` contacts. This endpoint allows 20 requests per minute per organization — see [Rate Limits](/api-reference/rate-limits).
</Note>
...
<ResponseField name="pagination" type="object">
  ...
  <Expandable title="Pagination object">
    <ResponseField name="page" type="number">
      Current page, derived from `offset` and `limit` (`null` when the request was not paginated)
    </ResponseField>
    <ResponseField name="total_rows" type="number">
      Total number of contacts matching the filters, across all pages
    </ResponseField>
    <ResponseField name="total_pages" type="number">
      Total number of pages for the requested `limit` (`null` when the request was not paginated)
    </ResponseField>
  </Expandable>
</ResponseField>
...
- `429 Too Many Requests` - Rate limit exceeded (this endpoint allows 20 requests per minute per organization); wait for `Retry-After` seconds. See [Rate Limits](/api-reference/rate-limits)
